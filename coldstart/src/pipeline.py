    ...
    for name in requested:
        if name == "ctrlite":
            ...
            cmcl_config = torch_backend.CMCLConfig(
                ...
                reg=float(cfg.get("reg", 1e-4)),
                iters=max(1, _coerce_int(cfg.get("iters"), 120)),
                ...
                sampled_negatives=max(0, _coerce_int(cfg.get("sampled_negatives"), 512)),
                neg_sampler_seed=_coerce_int(cfg.get("neg_sampler_seed"), seed),
                margin_m=float(cfg.get("margin_m", 0.1)),
                margin_alpha=float(cfg.get("margin_alpha", 0.1)),
                user_dropout=float(cfg.get("user_dropout", 0.2)),
                warmup_iters=max(0, _coerce_int(cfg.get("warmup_iters"), 5)),
                use_cosine_schedule=_coerce_bool(cfg.get("use_cosine_schedule"), True),
